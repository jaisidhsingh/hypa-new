...
class GHN3(GHN):
    ...
    def forward(self,
        ...
        if take_before_gnn and override_output:
            return x.squeeze(0)
        
        pre_gnn_embeddings = x.clone()
        pre_gnn_embeddings = pre_gnn_embeddings.squeeze(0)
        ...
        if not take_before_gnn and override_output:
            return x

        post_gnn_embeddings = x.clone()
        ...
        return (nets_torch, x, pre_gnn_embeddings, post_gnn_embeddings) if return_embeddings else (nets_torch, pre_gnn_embeddings, post_gnn_embeddings)
