#
# Taken from OpenCLIP: https://github.com/mlfoundations/open_clip/blob/main/src/training/scheduler.py
#
import numpy as np
...
def cosine_lr(optimizer, base_lr, warmup_length, steps): # linear warmup with cosine decay
    def _lr_adjuster(step):
        if step < warmup_length:
            lr = _warmup_lr(base_lr, warmup_length, step)
        else:
